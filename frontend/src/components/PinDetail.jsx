...
const Pindetails = ({user}) => {
  ...
  const {pinId} = useParams() //: means dynamic parameter
  ...
  if(!pinDetail) return <Spinner message="Loading WallPost"/>
  return (
    <>
      ...
      <div className="w-full p-5 flex-1 xl:min-w-620">
        ...
        <div className="flex flex-wrap mt-6 gap-3">
          ...
          <input className="flex-1 border-gray-100 outline-none border-2 p-2 rounded-2xl focus:border-gray-300"
            type="text"
            placeholder="Add your glorious comment here"
            value = {comment}
    ...
    {pins?.length > 0 && (
        <h2 className="text-center font-bold text-2xl mt-8 mb-4">
          Think you would like these too🤓
        </h2>
      )}
      {pins ? (
        <MasonryLayout pins={pins} />
      ) : (
        <Spinner message="Loading more Wallposts" />
      )}
    </>
  )
}
